...
final class session
{
    ...
    {
        if (isset($_SESSION['land']['is_login']))
        {
            self::$is_login  = true;
            self::$user_id   = $_SESSION['land']['user_id'];
            ...
            {
                self::$user_info = $_SESSION['land']['user_info'];
                self::$priv      = $_SESSION['land']['priv'];
                self::$n_unread  = $_SESSION['land']['n_unread'];
            }
    ...
    public static function set_login($user_id)
    {
        ...
        {
            $_SESSION['land']['is_login']  = true;
            self::$is_login             = true;
            
            $_SESSION['land']['user_id']   = $user_id;
    ...
    {
        $vcode = false;
        if (isset($_SESSION['land']['vcode']))
        {
            $vcode = $_SESSION['land']['vcode'];
            unset($_SESSION['land']['vcode']); //vcode只能用一次
        }
        return $vcode;
    }
    ...
    public static function get_user_info($do_update = false)
    {
        ...
        if ($do_update || !isset($_SESSION['land']['user_info']))
        {
            ...
            $_SESSION['land']['user_info'] = $user_info;

        }
        else
        {
            $user_info = $_SESSION['land']['user_info'];
        }
    ...
    public static function get_priv($do_update = false)
    {
        ...
        if ($do_update || !isset($_SESSION['land']['priv']))
        {
            ...
            $_SESSION['land']['priv'] = $priv;
        }
        else
        {
            $priv = $_SESSION['land']['priv'];
        }
    ...
    public static function get_n_unread($do_update = false)
    {
        ...
        if ($do_update || !isset($_SESSION['land']['n_unread']))
        {
            $conn = db_connect();
            fail_test($conn, false);
            ...
            $_SESSION['land']['n_unread'] = $n_unread;
        }
        else
        {
            $n_unread = $_SESSION['land']['n_unread'];
        }
